...
namespace RotatingShapes
{
    public class Icosahedron : IDisposable
    {
        ...
        private const float IcoScale = 0.55f; // Scale factor
        ...
        // Position this one bottom-center
        ...
        {
            // Different rotation
            _angle += (float)(deltaTime * 25.0f); // Slowest rotation
            ModelMatrix = Matrix4x4.CreateRotationX(Scalar.DegreesToRadians(_angle * 0.8f)) *
                          Matrix4x4.CreateRotationY(Scalar.DegreesToRadians(_angle * 1.1f)) *
        ...
        public unsafe void Render(Matrix4x4 viewMatrix, Matrix4x4 projectionMatrix)
        {
            ...
            _edgeShaderProgram.SetUniform("edgeColor", new Vector4(0.5f, 0.5f, 0.5f, 1.0f)); // Dark gray edges
            _gl.BindVertexArray(_edgeVao);
